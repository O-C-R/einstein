...
void newGraph() {
  ...
  println("done making new graph, total clusters: "+ clusters.size());
} // end newGraph

void draw() {
  // Update the physics world
  if (physicsOn) physics.update();
...
void keyPressed() {
  if (key == 'c') {
    ...
    physicsOn = !physicsOn;
    println("toggling physicsOn to: " + physicsOn);
  }
}
...
void mousePressed() {
  ...
  for (Cluster c : clusters) {
    for (Object o : c.nodes) {
      Node n = (Node)o;
      if (n.ptIsOver) {
        println("frameCount: " + frameCount + " over");
      }
    }
  }
} // end mousePressed
