...
// WIKIPEDIA ARTICLES
public void loadWikipediaArticles(String directory) {
  println("in loadWikipediaArticles for directory "+ directory);
  ArrayList<Wikipedia> newWikis = new ArrayList(); // in arrayList form
  HashMap<String, Wikipedia> newWikisHM = new HashMap(); // keep it in hm form too

  long startTime = millis();

  String[] allFileNames = OCRUtils.getFileNames(directory, false);
  ...
  int manualCounter = 0;
  for (String fileNameRaw : allFileNames) {
    String fileName = getFileName(fileNameRaw);
    boolean skip = false;
    for (String test : filesToSkip) if (fileName.toLowerCase().equals(test.toLowerCase())) skip = true;
    if (skip) continue;
    
    String[] broken = split(fileName, "_");
    String topic = broken[0].replace("-", " ").toLowerCase();
    String[] sources = new String[0];
    int year = 1000;
    try {
      year = Integer.parseInt(broken[1].trim());
    } 
    catch(Exception e) {
    }
    for (int i = 1; i < broken.length; i++) sources = (String[])append(sources, broken[i]);
    String[] text  = loadStrings(fileNameRaw);
    Wikipedia wiki = new Wikipedia(topic, sources, text, year);
    if (!newWikisHM.containsKey(topic)) {
      newWikisHM.put(topic, wiki);
      newWikis.add(wiki);
      println("loaded wiki fileName: " + fileName + " -- " + filesToSkip[0] + " for year: " + year);
    }

    //if (manualCounter++ >= 13) break; // ****** MANUAL BREAK ****** //
  }

  //for (Wikipedia w : newWikis) println(w);
  wikis = newWikis;
  wikisHM = newWikisHM;
} // end loadWikipediaArticles 
